...
const createorder = async function(req, res){
    try{
        ...
        const cartExist = await cartModel.findOne({_id: cartId, totalItems:{$gt:0}, userId: userId}).select({_id:0,__v:0})
        if(!cartExist){
            return res
                .status(400)
                .send({status: false, message:"No product exist in the cart to place an order, or Cart is not for the user logged in"})
        }
        ...
        return res
                .status(201)
                .send({status: true, message:"Successful", data: orderData})
    }
    catch(error){
        console.log(error)
        return res
...
const updateOrder = async function(req, res){
    try{
        ...
        let orderPlaced = await orderModel.findOne({_id: orderId})
